...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    if (!user) {
      throw new ApiError(
        500,
        "Unable to find user by ID during token creation"
      );
    }
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const avatarLocalPath = req.files?.avatar
    ...
    : ""; // in request object multer added "files" that why we can access "files" here
  ...
  const createdUser = await User.findById(user?._id || -1).select(
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const cookieOptions = {
    // now the cookie is only modified by server not fron fronend
    httpOnly: true,
    secure: true,
  };
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (incomingRefreshToken !== user?.refreshToken) {
      throw new ApiError(401, "Refresh Token Mis-match");
    }
